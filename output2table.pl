...
$latest_lhs = undef; # To track LHS symbol for bison v1.875 (1.75 also?) onwards
...
while (<>) {
    ...
    next if (/^\s+0\s+/); # rule 0 in bison>=1.75 (confirmed in 1.875)?
    ...
    if (m{^rule ([0-9]+) +([a-zA-Z0-9_]+) \-\>[ \t](.*)$}) {
        ...
        # Now match the two types of rule entry generated by bison 1.875 onwards
    } elsif (m{^\s+([0-9]+)\s+([a-zA-Z0-9_]+)\s*:\s+(.*)$}) {
        $number = $1;
        $latest_lhs = $lhs = $2;
        $rhs = $3;
    } elsif (m{^\s+([0-9]+)\s+\|\s+(.*)$}) {
